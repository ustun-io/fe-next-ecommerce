/** https://gist.github.com/ramirezsandin/cc93f6a405ff818e48038e28a26a1ba0 */
import { NextRouter, useRouter } from 'next/router'

import { ParsedUrlQuery } from 'querystring'

export const navigateTo = (router: NextRouter, url: string) => {
  router.push(url)
}
...
const useRouterParams = (options?: UseRouterParamsOptions) => {
   ...
   */
  const setParam = (name: string, value?: string | boolean | number | string[] | boolean[] | number[]) => {
    if (!value) {
      removeParam(name)
      return
    }
    ...
    reload(
      {
        ...
        query: {
          ...rest,
          [name]: Array.isArray(value) ? value.map((el) => encodeURIComponent(el)) : encodeURIComponent(value)
        }
      },
      undefined,
      { shallow }
   ...
   * @param value The value of the param.
   */
  const removeParam = (name: string, value?: string | number | boolean | string[] | number[] | boolean[]) => {
    const { [name]: param, ...rest } = query
